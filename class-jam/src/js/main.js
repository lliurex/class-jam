...
UI.prototype.exportAssembly = function exportAssembly(path) {
    try {
        ...
        var tmpdir = "/tmp" + dirname; // L'afegim dos vegades per comprimir el directori de dins
        fse.emptyDir(tmpdir, function () {
            ...
            fse.copy(self.configDir, tmpdir, { overwrite: true }, function () {
                ...
                //archive.directory('/tmp/kk', { name: 'kk' });
                archive.directory(tmpdir, {name: tmpdir});
    ...
    for (component in self.componentHelper) {
        ...
        if (item.componentactions) {
            ...
            if (typeof (item.componentactions) == "object") {
                ...
                currentcomponentactions = JSON.parse(item.componentactions);
            }
        }
        ...
        console.log("Afegint widget "+current+" de TamX="+x+" TamY="+y+" en col="+col+"row="+row);
...
UI.prototype.LaunchAssembly = function LaunchAssembly(id) {
	...
	if (!isEmpty) 
	  for (i of self.filedata){
            ...
            console.log(i.row);
            console.log(i.col);
            console.log(i.size_x);
            console.log(i.size_y);

            $("#"+i.component).attr("data-col", i.col);
            $("#"+i.component).attr("data-row", i.row);
